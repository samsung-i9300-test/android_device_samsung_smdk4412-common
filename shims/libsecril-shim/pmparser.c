...
void *pmparser_get_addr_start(int pid, const char *pathname, unsigned long length) {
	procmaps_iterator* maps = pmparser_parse(pid);
	if(maps == NULL) {
		ALOGE("%s: [map]: cannot parse the memory map of %d\n", __func__, pid);
		return (void*)-1;
	}

	//iterate over areas
	procmaps_struct* tmp = NULL;
	void *addr_start = (void*)-1;
	while( (tmp = pmparser_next(maps)) != NULL) {
		while(tmp != NULL) {
			if (!strcmp(tmp->pathname, pathname) && tmp->length == length) {
				addr_start = tmp->addr_start;
				goto out;
			}
			tmp=tmp->next;
		}
...
void pmparser_print(procmaps_struct* map, int order) {
	...
	while(tmp!=NULL) {
		...
		if(order==id || order==-1) {
			ALOGE("%s: Backed by:\t%s\n", __func__, strlen(tmp->pathname)==0?"[anonym*]":tmp->pathname);
			ALOGE("%s: Range:\t\t%p-%p\n", __func__, tmp->addr_start,tmp->addr_end);
			ALOGE("%s: Length:\t\t%ld\n", __func__, tmp->length);
			ALOGE("%s: Offset:\t\t%ld\n", __func__, tmp->offset);
			ALOGE("%s: Permissions:\t%s\n", __func__, tmp->perm);
			ALOGE("%s: Inode:\t\t%d\n", __func__, tmp->inode);
			ALOGE("%s: Device:\t\t%s\n", __func__, tmp->dev);
		}
		if(order!=-1 && id>order)
...
int pmparser_print_maps(){
	int pid=-1;

	procmaps_iterator* maps = pmparser_parse(pid);
	if(maps==NULL){
		ALOGE("%s: [map]: cannot parse the memory map of %d", __func__, pid);
		return -1;
	}
